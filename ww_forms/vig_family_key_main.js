...
function decode_sl(cl,cr,k, ciph_type) {
        ...
        if ( ciph_type == 1) { //BSLIDEFAIR
			if (cr == ((k-cl+26)%26)){ //cr,cl in vertical straight line
        ...
        else { // VIG/VAR SLIDEFAIR
			if (cl== ((26+cr-k))%26)	{ //cr,cl in vertical straight line
				pl = (cl+25)%26;
				pr = (cr+25)%26;
			}
			else {
...
function vig_family_decrypt(numeric_key,ciph_type){
    var str,i,j,k,n,s,c,cnt;
    var out_str;
    var key_len,index,offset;
	var cl,cr,pl,pr,result;
    ...
    //for (j=0;j<key.length;j++)
        //numeric_key[j] = l_alpha.indexOf( key.charAt(j) );
	...
	index = 0;
    var plain = [];
	if ( ciph_type<2){ // slidefair
		for (i = 0; i<code.length; i = i+2){
			result = decode_sl(code[i],code[i+1],numeric_key[index],ciph_type)
			plain[i] = result[0];
			plain[i+1] = result[1];
			if (++index == numeric_key.length)
				index = 0;
			
		}
	}
	else if ( ciph_type <= 9 && ciph_type >= 6 ){ // autokey
		for (i = 0; i<numeric_key.length; i++)
			plain[i] = decode_let(code[i],numeric_key[i],ciph_type)
		for(i= numeric_key.length;i<code.length;i++)
			plain[i] = decode_let(code[i],plain[i-numeric_key.length],ciph_type);

	}
	else{ // not slide fair or autokey
		for (i = 0; i<code.length; i++){
			plain[i] = decode_let(code[i],numeric_key[index],ciph_type)
			if (++index == numeric_key.length)
				index = 0;
		}
...
function do_solve(){
	...
	var x,y,n3,n4;
    var indx,state,k;
	var type_name, start_type,ciph_type;
	var numeric_key,key;
    ...
    for (i=0;i<word_list.length;i++){
		...
		plain_text = vig_family_decrypt(numeric_key,ciph_type);
