...
function get_trial_decrypt(){
	...
	for (x=0;x<buf_len;x++) {
		...
		// work_buffer[code_len++] = c1;
		// work_buffer[code_len++] = c2;
		// work_buffer[code_len++] = c3;
...
function get_crib_score(){
    ...
    if (crib_flag == 2){ // floating crib
        ...
        for ( crib_pos=0;crib_pos<buf_len-crib_len+1;crib_pos++)
            if ( score_buffer[crib_pos] == float_crib[0][0]) {
                     match = 0.0;
                    for (y=0;y<crib_len;y++)
                            if ( score_buffer[crib_pos+y] == float_crib[0][y]) {
                                    match += 1.0
                    }
                    if (match>best_match) {
                            best_match = match;
        ...
        if ( numb_cribs > 1) {
            ...
            for ( crib_pos=0;crib_pos<buf_len-crib_len+1;crib_pos++) {
                ...
                if ( score_buffer[crib_pos] == float_crib[1][0]) {
                        match = 0.0;
                        for (y=0;y<crib_len;y++)
                                if ( score_buffer[crib_pos+y] == float_crib[1][y]) {
                                        match += 1.0
                        }
                        if (match>best_match) {
                                best_match = match;
        ...
        if ( numb_cribs > 2) {
            ...
            for ( crib_pos=0;crib_pos<buf_len-crib_len+1;crib_pos++) {
                ...
                if ( score_buffer[crib_pos] == float_crib[2][0]) {
                        match = 0.0;
                        for (y=0;y<crib_len;y++)
                                if ( score_buffer[crib_pos+y] == float_crib[2][y]) {
                                        match += 1.0
                        }
                        if (match>best_match) {
                                best_match = match;
