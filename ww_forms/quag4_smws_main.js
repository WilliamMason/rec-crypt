...
function initialize(){
   ...
   hclimber = new Worker('quag4_smws_worker.js');
   ...
   if (numb_workers>1){
   		hclimber2 = new Worker('quag4_smws_worker.js');
   ...
   if (numb_workers>2){
   		hclimber3 = new Worker('quag4_smws_worker.js');
...
function do_solve(){
    ...
    // do_check() initializes crib string
	if (!do_check()) return;
	...
	str = str+':'+n;
	s = document.getElementById('max_c_len').value
	str += ':'+s;
	s = document.getElementById('max_p_len').value
	str += ':'+s;    
    str += ':'+crib_string;
    if ( document.getElementById('crib_bonus').checked)
        str += ':1';
    else str += ':0';
