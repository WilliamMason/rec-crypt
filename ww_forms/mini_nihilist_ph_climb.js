...
function get_score(period){
    ...
    var do_nihilist_calc = function(code){ // return this function which can use the pseudo-global variables
...
*/		
    ...
    } // end do_nihilist_calc function
    return (do_nihilist_calc); // return this function which can access pseudo_global variables
} // end nihilist_calcs closure function
...
function do_solve(){
	...
	*/
    do_nihilist_calc = nihilist_calcs(); // returns a function and initializes scoring table
    do_nihilist_calc(buffer);
...
debugger;
  ...
  period = Math.floor(Math.sqrt(buf_len));

	if ( buf_len != period*period){
		s="Text length is not a perfect square.";
		postMessage(s);
		return;
	}
