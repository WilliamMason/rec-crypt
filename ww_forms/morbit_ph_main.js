...
function initialize(){
   ...
   hclimber = new Worker('morbit_ph_climb.js');
   ...
   if (numb_workers>1){
   		hclimber2 = new Worker('morbit_ph_climb.js');
   ...
   if (numb_workers>2){
   		hclimber3 = new Worker('morbit_ph_climb.js');
...
function do_check(){
    ...
    for (var i = 0;i<s.length;i++){
        ...
        n = digits.indexOf(c);
        if ( n==0){
            alert("ciphertext contains a zero. Not Morbit!");
            return(false);
        }
        if ( n>0) s1 += c;
    }
    if (s1.length==0){
        alert("Ciphertext does not contain any digits! Not morbit!");
        return(false);
    }
	return(true);
}
