// keyphrase hill-climber, like keyhilltet.c, wimpy hill-climbing
...
function do_processing(str1,str2){ // str1 code, str2 crib
    ...
    var pos;
    var data_pattern,indx;
	var cycle_step, current_step,numb_accepted;

    //fudge_factor = 0.001; // temporary, later send this variable value
    ...
    for (i=0;i<str.length;i++){
        ...
        if ( space(c ) ) {
            ...
            n = alpha27.indexOf(c);
            if ( n == -1) {
              data_pattern.push(c); // non-ciphertext character
              continue;
            }
            space_flag = false;
            data_pattern.push(' ');
	...
	/* set up fixed part of l_array from crib */
    for (i=0;i<26;i++) fix_let[i] = 0;
	//memset(fix_let,0,sizeof(fix_let));
    for (i=0;i<26;i++) key_array[i] = -1;
	//memset(l_array,-1,sizeof(l_array));
    debugger;
    ...
    for (i=0; i<numb_free;i++)
        key_array[ free_let[i] ] = Math.floor(Math.random()*26);
    best_score = 0;
    debugger;
    ...
    for ( trial = 0;trial<max_trials;trial++){
        ...
        key_array[ free_let[pos] ] = Math.floor(Math.random()*26);
        score = get_score();
        if ( score > best_score){
            best_score = score;
            n = score.toFixed(2);
            //out_str = "Keyphrase plaintext:\n"
            out_str = '';
            //for (i=0;i<buf_len;i++)
            //    out_str += alpha27.charAt(plain_text[i]).toLowerCase();
            indx = 0;
            for(i=0;i<data_pattern.length;i++){
              if (data_pattern[i]!= ' ')
                out_str += data_pattern[i]
              else
                out_str += alpha27.charAt(plain_text[indx++]).toLowerCase();
            }
            out_str += '\nkeyphrase key\nabcdefghijklmnopqrstuvwxyz\n';
            for (i=0;i<26;i++)
                out_str += alpha27.charAt(key_array[i]);
...
function get_score() {
	...
	for (j=0;j<crib_len;j++)
		if ( crib_buffer[j] == plain_text[j])
			sum++;
	score += 100*sum;
	// score key phrase itself
    for (j=0;j<26-3;j++) {
        index = key_array[j]+27*key_array[j+1]
            +27*27*key_array[j+2]+27*27*27*key_array[j+3];
        score += tet27_table[index];
    }
