...
*/   
   var worker = new Worker('k4key_search_worker.js');
   ...
   if ( cnt != 26) {
     alert("Encrypting alphabet does not contain 26 symbols!");
     return;
   }
   ...
   worker.postMessage( {op_choice:1, word_list_string:word_list_string});
   //worker.webkitPostMessage( {op_choice:2, str:str});
   worker.postMessage( {op_choice:2, str:str});
}

function do_clear(){
    document.getElementById('word_pattern').value = '';
}
onload = function() {
