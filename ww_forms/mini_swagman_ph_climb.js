...
function get_trial_decrypt(period){
	...
	count = index = 0;
	for ( col = 0; col <numb_cols;col++) {
        for (row=0;row<period;row++)
			plain_text[col+key[index][row]*numb_cols] = buffer[count++];
        index++;
...
*/		
       ...
       for (period = min_period; period <= max_period; period++){
        ...
        n3 = Math.floor(Math.random()*period);
		v1 = key[n3][n1];
		v2 = key[n3][n2];
		key[n3][n1]=v2;
		key[n3][n2]=v1;
    ...
    } // end do_swagman_calc function
    return (do_swagman_calc); // return this function which can access pseudo_global variables
...
*/	
    do_swagman_calc = swagman_calcs(); // returns a function and initializes scoring table
    do_swagman_calc(buffer);
