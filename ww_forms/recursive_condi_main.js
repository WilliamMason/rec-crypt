...
function do_processing(){
	...
	n = 27*27-1; // first row has just 26 letters 
	if (start_key.length == n){ // remove header row and column
		temp = '';
		// remove header row and columnscolumn
		for (i=27;i<start_key.length; i = i+27)
			temp += start_key.substring(i,i+26)
		start_key = temp;
	}
	n = 26*26
	if ( start_key.length != n){
		alert('Initial key table does not have 26x26 letters and dashes');
		return;
	}
	xfer = {};
	...
	if ( document.getElementById('current_solutions').checked )
		xfer["all_solutions_flag"] = true;
	else
		xfer["all_solutions_flag"] = false;
...
function initialize_worker(){
	...
	var show_update = 1000;

    
   hworker = new Worker('recursive_condi_worker.js');
   hworker.onmessage = function (event) {
    out_str = event.data
	if (out_str == 'timer:'){
		words_processed += processed_chunk;
		if (words_processed > show_update)
			document.getElementById('numb_tries').value = words_processed;
	}
	else {
		words_processed = 0;
		document.getElementById('numb_tries').value = words_processed;
...
function do_reset(){ // reset key table
	var s,i,j;
...
document.getElementById('start_key').value = s;
}
