...
function initialize(){
   ...
   hclimber = new Worker('expand_key_climb.js');
   ...
   if (numb_workers>1){
   		hclimber2 = new Worker('expand_key_climb.js');
   ...
   if (numb_workers>2){
   		hclimber3 = new Worker('expand_key_climb.js');
...
function do_check(){
	...
	s = document.getElementById('original_key').value;
	if (s == ''){
		alert("No original key entered");
		return(false)
	}
	s = s.toLowerCase();
	s1=''
	...
	n = parseInt(s);
	if (n<s1.length){ // keep max length at least as big as original key length
	  n = s1.length;
	  c = ''+n;
	  document.getElementById('max_key').value = c;
	}
...
function do_solve(){
	...
	str = str+':'+n;
	max_key1 = document.getElementById('max_key').value;
	str = str+':'+max_key1;
	...
	str = document.getElementById('original_key').value;
	hclimber.postMessage(str);
...
function do_clear(){
	...
	document.getElementById('original_key').value = '';
