...
function get_cipher_type() {
	...
	if ( cipher_id[working_on_numb].charAt(0) == 'A')
		cipher_type = 0;
	else cipher_type = 1;
}
...
function initialize_display(){
    ...
    document.getElementById('aca_format').addEventListener("click",output_aca_solutions);   
	//id_display();
	//solving_flag=1;
	s="Key: <br>";
...
function save_to_disk(){
	...
	if (typeof(localStorage) == 'undefined' ) {
		...
		try {
			localStorage.setItem("ap_batch_7880.numb_ciphers", numb_ciphers); //saves to the database, “key”, “value”
		} catch (e) {
...
function check_storage(){
	...
	s = localStorage.getItem("ap_batch_7880.numb_ciphers");
	if (s != undefined){
...
function output_aca_solutions(){
	var s,str,pl,sol_numb,i,j;
    var aca_line_len = 65;
    var min_characters = 50;
...
if (show_sols_flag==0){
	...
	stored_col_orders[working_on_numb]=s;		
	//set up display
	s='<br><span style="font-weight:bold;">Solutions (ACA format): </span> (to copy output to clipboard, ';
	s += 'click inside box, then Ctl-A to highlight, then Ctl-C to copy):<br>';
    s+='<textarea id="show_solutions" style="font-family: monospace;" name="solutontext" rows="6" cols="90">';
    s+='</textarea><br>';
    document.getElementById('solution_space').innerHTML= s;	
	location.href = "#button_actions"; // scroll down a little so solutions
    str='Solutions (ACA format):\n\n';
 for (sol_numb=0;sol_numb<numb_ciphers;sol_numb++){  
	if(keys[sol_numb] == empty_key) continue;
    str += cipher_id[sol_numb]+' ';
		sol_code = ciphers[sol_numb];
	...
	pos=0
	while ( pos < sol_code.length ) {
		limit = pos+aca_line_len;
		if ( limit >= sol_code.length)
			limit = sol_code.length-1;
		else {
			while ( break_pt(sol_code.charAt(limit))== 0 && limit>min_characters )
				limit = limit -1;
			if (limit == pos) //no place to split!
				limit = pos+aca_line_len;
		}
        ...
        // reformat Pl
        pl = trim(pl); // remove leading, tailing, and multiple spaces.
        pl = pl.replace(/\*./gi,do_proper);
        pl = pl.replace(/=/g,"-"); // equal signs by hyphens
        pl = capitalize(pl); // capitalize first character
		str = str+pl+"\n"
		break; // just output one line
	}
 }// next sol_numb
	document.getElementById('show_solutions').value=str;
}
...
function do_proper(s){ // capitalize letter preceeded by an asterisk (proper noun), & remove asterisk
    var c;
    c = s.charAt(1).toUpperCase();
    return(c);
}
