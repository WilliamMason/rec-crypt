...
function initialize(){
   ...
   hclimber = new Worker('serplayfair_ph_climb.js');
   ...
   if (numb_workers>1){
   		hclimber2 = new Worker('serplayfair_ph_climb.js');
   ...
   if (numb_workers>2){
   		hclimber3 = new Worker('serplayfair_ph_climb.js');
...
function do_check(){
	...
	if (s1.indexOf('j') != -1 && flag6x6 == false){
		alert("Ciphertext has a J!");
		return(false);
	}
	s = document.getElementById('s_period').value;
	if (s == ''){
    ...
    if ( crib_status_flag == 1) {
        ...
        if (flag6x6) symbols = symbols6;
        else symbols = symbols5;
        n=0;
        s1 = '';
