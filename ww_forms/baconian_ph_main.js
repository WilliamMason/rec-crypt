...
function initialize(){
   ...
   hclimber = new Worker('baconian_ph_climb.js');
   ...
   if (numb_workers>1){
   		hclimber2 = new Worker('baconian_ph_climb.js');
   ...
   if (numb_workers>2){
   		hclimber3 = new Worker('baconian_ph_climb.js');
...
function do_solve(){
	...
	str = str+':'+n;
    if (document.getElementById('word_list_scoring').checked)
        str += ':1';
    else
        str += ':0';
...
function initialize_crib() {
	...
	if ( (s1.length%5) !=0 ){
		alert("Ciphertext length not divisible by 5!");
		return;
	}
    n = s1.length;
    plain_len = Math.floor(n/5);
