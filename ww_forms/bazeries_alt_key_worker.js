...
// alternative way of spelling the key
...
function xlate_baz(numb_str){
    ...
    if ( word_len>4){
        alert("Alternatice key must be less than a ten thousand!");
        return('Q');
    }
    to_go = word_len;
    txt='';
    while(to_go > 0){
		if (to_go==3||to_go==1) // in [3,1]:
...
onmessage = function(event) { //receiving a message
  ...
  else if (state == 3){ // should be only possibility
    str = event.data.str;
    if ( use_6x6 == 0)
        do_solve(str,1111,10000)
    else
        do_6x6_solve(str,1111,10000)    
  }
  else if (state == 4){
    use_6x6 = 1;
  }
