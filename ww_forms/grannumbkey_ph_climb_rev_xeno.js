// PH hill-climber with log pentagraph and reverse word/ phrase list scoring
...
function get_score(){
	...
	word_score = 0;
	while(pos >= 0){
		n = rev_word_search(pos,buf_len)[0];
		if ( n> CUT_OFF){
		...
		//score = get_score();
       	if (score > current_hc_score-fudge_factor*buf_len/(noise_level)) {
            ...
            if ( score>max_score){
                ...
                for (i=0;i<buf_len;i++)
                    out_str += alpha.charAt(plain_text[i]).toLowerCase();
                out_str += "\nreverse score: "+score.toFixed(2)+" on trial: "+trial;
