...
function do_hill_climbing(str){
    ...
    for (i=0;i<period;i++)
        period_key[i] = 10*(Math.floor( Math.random()*5)+1)+ Math.floor( Math.random()*5)+1;
	...
	for (trial = 0;trial < max_trials;trial++){
        ...
        if (op_choice<50) {
            ...
            v1 = period_key[n1];
            period_key[n1] = 10*(Math.floor( Math.random()*5)+1)+ Math.floor( Math.random()*5)+1;
        }
		...
		if ( score>max_score){
            ...
            used_key_letters[9] = true; // skip J
            ...
            for (i=0;i<period;i++) {
                ...
                y = (period_key[i]%10)-1;
                out_str += alpha.charAt(key[5*x+y])+') ';
                used_key_letters[ key[5*x+y] ] = true;
            }
