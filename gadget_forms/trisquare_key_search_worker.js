...
var ciphertext, plaintext,left_key,top_key, middle_key;
...
function do_calc(){
    ...
    var key1,key2,best_score,score,route_num, best_route,key3;
    var cipher1, plain1,cipher2,plain2,s,cipher3;
    var op_choice;
    ...
    if (key1.length != 25) {
        str = "Left Key does not have 25 symbols!"
    ...
    if (key2.length != 25) {
        str = "top Key does not have 25 symbols!"
        postMessage( {op_choice:1, str:str});
        return;
    }
    n = 0;
    ...
    if (key3.length != 25) {
        str = "Middle Key does not have 25  symbols!"
        postMessage( {op_choice:1, str:str});
        return;
    }
    n = 0;
    ...
    */
    ...
    for (i=0;i<s.length;i = i+3){
        cipher1[n] = s.charAt(i); // left square
        cipher3[n] = s.charAt(i+1); // middle square
        cipher2[n] = s.charAt(i+2); // top square
        n++;
    }
    ...
    if ( (cipher1.length != plain1.length)||(cipher2.length != plain2.length)){
        ...
        str = "Ciphertext and Plaintext have different lengths!(check for overlooked plaintext dashes)"
        postMessage( {op_choice:1, str:str});
        return;
    }
    ...
    var missing_top = ". unused: ";
    var missing_middle = ". unused: ";
    for (i=0;i<5;i++) for (j=0;j<5;j++){
		c = sq1[i][j]
		if (cipher1.indexOf(c) == -1 && plain1.indexOf(c) == -1){
			sq1[i][j] = '-'
            missing_left += c;
        }
    }
    for (i=0;i<5;i++) for (j=0;j<5;j++){
		c = sq2[i][j]
		if (cipher2.indexOf(c) == -1 && plain2.indexOf(c) == -1){
			sq2[i][j] = '-'
            missing_top += c;
        }
    }
    ...
    var best_top_column_order;
    var best_middle_row_order;
    var best_middle_column_order;
    ...
    out_str2 += '\nbest score for top keysquare is '+best_score+' with route '+route_name[best_route];
	//document.getElementById('output_area').value = out_str2;

    var swap_array_row = [0,0,0,0,0];
    var key_order_column = [0,1,2,3,4];
    var swap_array_col;
    var key_order_col;
    var newsq = []
    best_top_column_order = key_order_column.slice(0);
    // now try all equivalent squares
    while( next_permutation(key_order_row,swap_array_row,5) != 0 ){
        key_order_col = [0,1,2,3,4];
        swap_array_col = [0,0,0,0,0];
        ...
        for (route_num = 0;route_num < routes.length;route_num++){
            ...
            if (score > best_score){
                ...
                out_str2 += '\nbest score for top keysquare is '+best_score+'\nroute: '+route_name[best_route];
                //document.getElementById('output_area').value = out_str2;
                best_top_column_order = key_order_col.slice(0);
            }
        }
        while( next_permutation(key_order_col,swap_array_col,5) != 0 ){
            get_next_square(newsq,sq2,key_order_row,key_order_col,5)
    ...
    str = out_str2+missing_top;
    postMessage( {op_choice:3, str:str});
    
    // now do middle keysquare
    var out_str3 = '';
    best_score = score_keysquare(sq3,0)
    best_route = 0;
    // try all routes in original square
    for (route_num = 1;route_num < routes.length;route_num++){
        score = score_keysquare(sq3,route_num);
        if (score>best_score){
            best_score = score;
            best_route = route_num;
        }
    ...
    out_str3 += '\nbest score for middle keysquare is '+best_score+' with route '+route_name[best_route];
	//document.getElementById('output_area').value = out_str2;

    var swap_array_row = [0,0,0,0,0];
    var key_order_row = [0,1,2,3,4];
    var swap_array_col;
    var key_order_col = [0,1,2,3,4];
    var newsq = []
    best_middle_row_order = key_order_row.slice(0);
    best_middle_column_order = key_order_col.slice(0);
    // now try all equivalent squares
    while( next_permutation(key_order_row,swap_array_row,5) != 0 ){
        key_order_col = [0,1,2,3,4];
        swap_array_col = [0,0,0,0,0];
        get_next_square(newsq,sq3,key_order_row,key_order_col,5)
        for (route_num = 0;route_num < routes.length;route_num++){
            score = score_keysquare(newsq,route_num)
            if (score > best_score){
                best_score = score
                ...
                out_str3 += '\nbest score for middle keysquare is '+best_score+'\nroute: '+route_name[best_route];
                //document.getElementById('output_area').value = out_str2;
                best_middle_row_order = key_order_row.slice(0);
                best_middle_column_order = key_order_col.slice(0);
            }
    ...
    for (i=0;i<5;i++)
        if (best_left_row_order[i] != best_middle_row_order[i]){
            s = '\nWARNING: Left and middle row orders are inconsistent';
            break;
    }


    for (i=0;i<5;i++)
        if (best_top_column_order[i] != best_middle_column_order[i]){
            s += '\nWARNING: top and middle column orders are inconsistent';
            break;
    }

    //document.getElementById('output2_area').value = out_str2+missing_top+s;
    str = out_str3+missing_middle+s;
    postMessage( {op_choice:4, str:str});
...
onmessage = function(event) { //receiving a message
  ...
  top_key = event.data.top_key;
  middle_key = event.data.middle_key;
  do_calc();
}
